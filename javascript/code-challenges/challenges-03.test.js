...
const notInFirstArray = (forbiddenValues, arr) => {
  return arr.filter((element) => {
    ...
    forbiddenValues.forEach((forbiddenElement) => {
      if (forbiddenElement === element) {
        matchFound = true;
      };
    })
    if (!matchFound) {
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  return arr.filter((statInfo) => {
    if (statInfo.baseStat > minBaseStat) {
      return statInfo;
    }
  })
};
...
const getStatName = (arr, minBaseStat) => {
  ...
  arr.filter((statInfo) => {
    if (statInfo.baseStat > minBaseStat) {
      newArr.push(statInfo.stat.name);
    }
  })
  return newArr;
};

// why does the below version of the function return the whole stat object?

// return arr.filter((statInfo) => {
//   if (statInfo.baseStat > minBaseStat) {
//     return statInfo.stat.name;
//   }
// })
