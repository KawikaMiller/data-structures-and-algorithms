...
function transformToLis(obj){
  ...
  keys.forEach(key => {
    listPairs = [...listPairs, `<li>${key}: ${values[keys.indexOf(key)]}</li>`]
  })
...
const totalSum = (input) => {
  return input.flat(Infinity).reduce((acc, current) => acc + current, 0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
    return input
      .map(element => element.filter(item => {
        if (typeof item === 'number' && item % 5 === 0) {
          return item
        }
      }))
      .map(element => element.map(item => Math.pow(2, item)))
};
...
let findShortest = (data) => {
  return data.reduce((acc, currentValue) => {
    if (acc.height > currentValue.height) {
      return acc;
    } else if (acc.height < currentValue.height) {
      return currentValue;
    }
  }).name
};
